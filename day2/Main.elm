...
diff input =
    let
        ...
        diffLines =
            List.map diffLine lines
    in
        List.foldr (+) 0 diffLines
...
max list =
    let
        intList =
            String.words list |> List.map (\x -> String.toInt x |> Result.withDefault 0)
    in
        List.maximum intList |> Maybe.withDefault 0
...
min list =
    let
        intList =
            String.words list |> List.map (\x -> String.toInt x |> Result.withDefault 0)
    in
        List.minimum intList |> Maybe.withDefault 0
